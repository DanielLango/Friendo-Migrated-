...
import { updatePartnerVenue } from '../utils/adminVenueManager';
...
export default function AdminEditVenueScreen() {
  const route = useRoute();
  const navigation = useNavigation();
  const { colors } = useTheme();
  
  const venue = (route.params as any)?.venue;
  ...
  const [partnershipLevel, setPartnershipLevel] = useState<'basic' | 'premium' | 'featured'>(venue?.partnership_level || 'basic');
  const [specialOffer1, setSpecialOffer1] = useState(venue?.special_offers?.[0] || '');
  const [specialOffer2, setSpecialOffer2] = useState(venue?.special_offers?.[1] || '');
  const [rating, setRating] = useState(venue?.rating?.toString() || '');
  const [priceLevel, setPriceLevel] = useState(venue?.price_level?.toString() || '');
  const [isFeatured, setIsFeatured] = useState(venue?.is_featured || false);
  const [displayOrder, setDisplayOrder] = useState(venue?.display_order?.toString() || '0');
  ...
  const handleSave = async () => {
    ...
    const result = await updatePartnerVenue(venue.id, updates);
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <View style={[styles.header, { backgroundColor: colors.cardBackground, borderBottomColor: colors.border }]}>
        ...
        <Text style={[styles.headerTitle, { color: colors.text }]}>Edit Venue</Text>
